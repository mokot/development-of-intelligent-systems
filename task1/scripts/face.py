...
class FaceLocalizer:
    ...
    def find_faces(self):
        ...
        for face_rectangle in face_rectangles:
            ...
            def get_neki(t_x1, t_x2, t_y1, t_y2):
                face_distance = float(np.nanmean(
                    depth_image[t_y1:t_y2, t_x1:t_x2]))

                return self.get_pose(
                    (t_x1, t_x2, t_y1, t_y2), face_distance, depth_time)

            x_pol = (x1 + x2) // 2

            a = get_neki(x1, x_pol, y1, y2)
            b = get_neki(x_pol, x2, y1, y2)
            ...
            if (a is not None and b is not None):
                ...
                a = np.multiply(get_smth(a.position), 1)
                b = np.multiply(get_smth(b.position), 1)
                ...
                dir_quat = R.from_rotvec([dy, dx, 0]).as_quat()

                stamp = rospy.Time(0)
                header = Header(stamp=stamp, frame_id='map')

                marker_arr = [
                    Marker(header=header, action=Marker.DELETEALL)]

                p = pose.position

                scnd = np.add(np.multiply([dy, -dx, 0], 5), [p.x, p.y, 0])

                s = Pose(position=Point(
                    x=scnd[0], y=scnd[1], z=pose.position.z))

                point = pose

                # point.orientation.x = dir_quat[0]
                # point.orientation.y = dir_quat[1]
                # point.orientation.z = dir_quat[2]
                # point.orientation.w = dir_quat[3]

                marker_arr.append(Marker(header=header,
                                         points=[point.position, s.position],
                                         type=Marker.ARROW,
                                         action=Marker.ADD,
                                         frame_locked=False,
                                         id=0,
            ...
            if len(encoding) == 0:
                print(
                    f'{Colors.WARNING}> dLib found a face, but face_recognizer didn\'t {Colors.ENDC}')
                continue
            else:
                encoding = encoding[0]
